...
//  cocao
//
//  Created by Christopher Cao on 2/18/18.
...
class CognitoConfig: NSObject {
    ...
    func getIdentityPoolId() -> String {
        let identityPoolId = self.keys?["identityPoolId"] as? String
        guard identityPoolId != nil else {
            fatalError("You must specify an identityPoolId in your CognitoConfig.plist file")
        }
    ...
    func getAppId() -> String {
        let appId = self.keys?["appId"] as? String
        guard appId != nil else {
            fatalError("You must specify an appId in your CognitoConfig.plist file")
        }
        return appId!
    }
